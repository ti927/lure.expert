...
/**
 * Data em que `match_count` ganhou um escritor.
 *
 * A coluna existe desde a Fase 1 e **nunca foi incrementada** — 518 regras no
 * banco, 518 com zero. Zero, portanto, nunca significou "não pegou nada": era o
 * valor de todas elas. Uma leitura do MCP concluiu, com toda a lógica correta,
 * que "500 de 500 regras nunca foram aplicadas" — resposta que teria saído
 * idêntica se cada uma delas pegasse mil lançamentos por dia.
 *
 * Agora o job soma. Mas o passado não volta: numa regra anterior a esta data o
 * contador conta só o que houve DEPOIS, e é por isso que cada linha declara se o
 * próprio contador é confiável, em vez de deixar quem lê supor que é.
 *
 * A borda é conservadora — um dia à frente do deploy — porque regra criada mais
 * cedo no mesmo dia ainda é anterior ao escritor.
 */
export const CONTADOR_VIVO_DESDE = '2026-08-24'
...
export interface RegraResumida {
  ...
  /** `false` em regra anterior a `CONTADOR_VIVO_DESDE`: o 0 dela não prova nada. */
  contadorConfiavel: boolean
  criadaAutomaticamente: boolean
}

export interface ListaDeRegras {
  regras: RegraResumida[]
  /** O total REAL na organização, não o tamanho desta página. */
  total: number
  offset: number
  /** Há mais para buscar com offset maior? */
  temMais: boolean
  /** Quantas das listadas têm contador em que não se pode confiar. */
 ...
 *
 * A tela tem a sua própria leitura em `listRules`, paginada por página; esta é a
 * forma que o MCP precisa — nome em vez de id (um uuid não diz nada a quem lê a
 * resposta) e `offset`, porque uma organização daqui já tem 510 regras e sem
 * offset as 10 últimas seriam invisíveis sem nenhum sinal disso.
 */
export async function listarRegras(
  ...
  const limite = Math.min(opcoes.limite ?? 200, 500)
  const offset = Math.max(opcoes.offset ?? 0, 0)
  ...
  // O total é contado, não deduzido do tamanho da página. Devolver
  // `regras.length` como "total" é pior que não devolver total nenhum: quem lê
  // conclui que viu tudo, e o número parece confirmar.
  ...
  const linhas = await db
    .select({
      ...
      criadaEm: categorizationRules.createdAt,
    ...
    // `{ field, op, value }` ficaram invisíveis na tela desde a 0019 e não faria
    // sentido o MCP enxergar o que a tela não deixa editar.
    .where(filtro)
    // Ordem estável, e por isso o offset é confiável: `updated_at` empata com
    // frequência (um lote de regras nasce no mesmo instante), e sem o desempate
    // por `id` duas páginas seguidas poderiam repetir e pular linhas.
    .orderBy(desc(categorizationRules.updatedAt), desc(categorizationRules.id))
    .limit(limite)
    .offset(offset)

  const corte = new Date(`${CONTADOR_VIVO_DESDE}T00:00:00Z`)
  const regras: RegraResumida[] = linhas.map(l => {
    ...
    return {
      ...
      contadorConfiavel: l.criadaEm >= corte,
  ...
  return {
    ...
    total,
    offset,
    temMais: offset + regras.length < total,
    contadorNaoConfiavel: regras.filter(r => !r.contadorConfiavel).length,
  }
}

/**
 * Soma `match_count` — uma instrução por bloco, não uma por lançamento.
 *
 * Um UPDATE por lançamento seriam 50 idas ao banco por bloco do job, e 7.762
 * numa importação grande; agregar no processo e mandar um
 * `UPDATE ... FROM (VALUES ...)` custa uma.
 *
 * `updated_at` fica de FORA de propósito: a tela e o MCP ordenam a lista de
 * regras por ele, e tocá-lo aqui faria toda importação embaralhar a ordem que a
 * pessoa usa para achar o que editou por último.
 *
 * Falha aqui não derruba a categorização: o contador é informativo, e perder uma
 * contagem vale muito menos que perder o lote inteiro de classificações.
 */
export async function somarMatchCount(
  organizationId: string,
  contagens: Map<string, number>,
): Promise<void> {
  if (contagens.size === 0) return
  const pares = Array.from(contagens.entries())
  try {
    await db.execute(sql`
      UPDATE categorization_rules AS r
         SET match_count = r.match_count + v.n
        FROM (VALUES ${sql.join(pares.map(([id, n]) => sql`(${id}::uuid, ${n}::int)`), sql`, `)})
             AS v(id, n)
       WHERE r.id = v.id AND r.organization_id = ${organizationId}::uuid
    `)
  } catch (e) {
    console.error('[regras] falha ao somar match_count', (e as Error).message)
  }
}
