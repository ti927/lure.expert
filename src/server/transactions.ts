...
import { db } from '@/db'
import { memberships, transactions, categorizationRules, categories, documents, costCenters, businessUnits, legalEntities, contacts, dataSources } from '@/db/schema'
...
export async function classifyTransaction(id: string, data: DimensionData) {
  ...
  if (parsed.data.contactId !== undefined) updates.contactId = parsed.data.contactId
