...
export function OnboardingForm({ temConvites }: { temConvites: boolean }) {
  const [state, formAction] = useFormState(createOrganization, { error: null })

  return (
    <Card className="w-full max-w-md shadow-md">
      <CardHeader className="space-y-1">
        {!temConvites && <div className="mb-2 text-2xl font-bold text-primary">lure.expert</div>}
        <CardTitle className="text-xl">{temConvites ? 'Ou crie sua própria empresa' : 'Bem-vindo'}</CardTitle>
        <CardDescription>
          {temConvites
            ? 'Se preferir começar do zero em vez de aceitar um convite.'
