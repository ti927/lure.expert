...
| `query/spec.ts` | `querySpecSchema` — **um Zod, três consumidores**: motor, ferramenta MCP `consultar` e bloco de dashboard. **Campo com semântica não óbvia exige `.describe()`**: JSDoc não chega ao modelo, e o que ele não lê ele conclui que não existe |
