...
**⚠️ Pendência ativa de 26/ago: aplicar a migration `0032_alerta_saldo_negativo.sql`.** Ela
acompanha o corte do saldo e **não é opcional**: `REGRAS_DE_ALERTA` é validado na leitura, e os 3
blocos `alertas` já gravados guardam `'saldo-negativo'` por extenso — sem a migration, o bloco de
alertas dos painéis materializados vem com `erroDeSpec` na tela.
...
**Do corte do saldo (26/ago), depois de aplicar a 0032:** em `/dashboard`, a linha de KPIs tem
**três** cartões e nenhum de saldo, e o bloco de **alertas** continua funcionando (é o que a
migration protege — se vier com erro de spec, ela não foi aplicada). Em `/fluxo`, a tela é só a
...
- 🔲 `db/migrations/rls/0032_alerta_saldo_negativo.sql` — **A APLICAR.** Não mexe em estrutura
  nenhuma: tira `'saldo-negativo'` do `jsonb` das specs de bloco `alertas` já gravadas. É
  obrigatória junto do deploy — `REGRAS_DE_ALERTA` é validado **na leitura** também, então sem ela
  os blocos de alerta dos painéis materializados passariam a vir com `erroDeSpec`. Duas sentenças, e
  **a ordem importa**: a 1ª apaga a chave `regras` de quem só tinha essa (lista vazia falharia no
  ...
  preservando a ordem. Validada com ROLLBACK (13/13) contra o banco real, incluindo o teste que dá
  sentido a ela — a spec resultante volta a passar por `lerBlockSpec`, e a spec ANTIGA é recusada
  pelo enum novo. Idempotente
