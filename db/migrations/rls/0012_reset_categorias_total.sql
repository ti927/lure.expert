-- Migration 0012: reset total das categorias e reseed em todas as orgs.
--
-- Contexto: 2 orgs criadas antes da migration 0009 ficaram com plano de contas
-- incompleto (a 0009 só remapeou tipos, não inseriu categorias novas). Em vez
-- de backfill cirúrgico, fazemos wipe total e reseed do zero com a estrutura
-- atual (definida na 0011: emprestimos_amortizacoes + investimentos_retiradas).
--
-- Autorização explícita do usuário: dados são todos de teste, pode apagar tudo.
--
-- Refactor permanente: extrai o corpo do seed numa função reusável
-- `seed_categories_for_org(uuid)`. Dali em diante, qualquer reseed manual vira
-- `SELECT seed_categories_for_org('<org_uuid>');` no SQL Editor. A trigger
-- function `seed_default_categories()` passa a apenas delegar.
--
-- Rodar no Supabase Studio > SQL Editor.

BEGIN;

-- ─── 1. Refactor: extrair seed numa função reusável ─────────────────────────
...
-- Trigger function passa a só delegar pra função reusável.
CREATE OR REPLACE FUNCTION seed_default_categories()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  PERFORM seed_categories_for_org(NEW.id);
  RETURN NEW;
END;
$$;

-- A trigger trg_seed_categories_on_org_create já aponta para
-- seed_default_categories(), não precisa recriar — o CREATE OR REPLACE acima
-- atualizou a função sem mexer no trigger.

-- ─── 2. Wipe total dos dados vinculados a categorias + categorias ───────────
...
-- ─── 3. Reseed: chama seed_categories_for_org pra cada org ──────────────────

SELECT seed_categories_for_org(id) FROM organizations;
